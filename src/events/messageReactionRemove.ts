...
export async function execute(bot: Bot, reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser) {
  ...
  console.log(`Reaction removed - Emoji ID: ${emojiId}, User: ${user.username}`);
  ...
  try {
    ...
    if (member.roles.cache.has(roleId)) {
      ...
      console.log(`❌ Removed role "${role.name}" from ${user.username} (Emoji ID: ${emojiId})`);
    } else {
      console.log(`User ${user.username} doesn't have role "${role.name}"`);
    }
  } catch (error) {
